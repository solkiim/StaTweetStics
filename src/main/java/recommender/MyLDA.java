...
// 	public MyLDA(double inAlpha, double inBeta ,double inGamma,  int inNumTopics, 
...
// 					sumTopicWordCount[topic] -= 1;
// 					for (int tIndex = 0; tIndex < numTopics; tIndex++) {
// 						multiPros[tIndex] *= ((topicWordCount[tIndex][word] + beta + 1) / (sumTopicWordCount[tIndex] + beta*fbCount[1] + 1));
// 					}
...
// 		fbProbs[0] *= (backCount[word] + betaBackground)/(fbCount[0] + betaBackground*vocabularySize);
// 		fbProbs[1] *= (topicWordCount[topic][word] + beta)/(sumTopicWordCount[topic] + beta*vocabularySize);
