...
public class WFRanker<V extends Vertex<V,E>, E extends Edge<V,E>>
 implements Ranker<V> { 
	/**
	...
	*/
	...
	public WFRanker(double dampingFactor, double precision) { 
	...
	protected void iteration() { 
		double dampingTerm = (1 - dampingFactor) / vertices.size(); 
		//newRanks.clear(); 
		////////////////////////here///////////////////
		double danglingRank = 0; 
		for (PRNode node : vertices.values()) { 
			//Node node = graph.getNode(i); 
			double sum = 0;
			Set<V> others = node.others();
			for (V other : others) {
				PRNode othern = vertices.get(other);
			 	sum += othern.cost()/othern.outDegree();
			 	System.out.println(other+" outDegree: "+othern.outDegree());
			}
			node.setNewRank(dampingTerm*node.vert().nodeWeight() + dampingFactor*node.vert().nodeWeight() * sum);
		...
		double hi = (double) vertices.size();
		danglingRank *= dampingFactor / hi;
		System.out.println("damping factor: "+dampingFactor+" size "+hi+" danglingRank: "+danglingRank);
		normDiff = 0; 
		for (PRNode node : vertices.values()) { 
			//Node node = graph.getNode(i); 
