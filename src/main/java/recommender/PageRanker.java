 ...
 implements Ranker<V> { 
	...
	*/
	...
	public void init(List<V> vertexs) { 
		...
		double initialRank = 1.0 / vertexs.size(); 
		for (V vert : vertexs) {
			vertices.put(vert,new PRNode(vert,initialRank)); 
		}
	...
	public void compute() { 
		...
		do { 
			iteration(); 
			if (DEBUG_OUTPUT) {
				System.out.println(String.format("%6d%16.8f", iterationCount, normDiff)); 
			}
	...
	protected void iteration() { 
		...
		for (PRNode node : vertices.values()) { 
			...
			node.setNewRank(dampingTerm + dampingFactor * sum);
